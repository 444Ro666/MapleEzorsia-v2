...
const char* MainMain::m_sRedirectIP = "127.0.0.1";			//custom exp table and level numbers MUST MATCH server numbers
const char* MainMain::m_sOriginalIP = "63.251.217.2";	//paste your custom exp table's array into the place of v83ArrayForCustomEXP
...
//fast so favor lower numbers. if the number is too high the non-system hooks wont reach the main thread in time and the main thread will
	Sleep(sleepySleepy);//run without them (causing hackshield load that is normally overwritten and hooked in time to block access)
